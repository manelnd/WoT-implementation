...
import { KeyboardButton, MouseButton, Position, Size, Image, KeyboardRegularButton, TextToImgOptions } from "../types";
...
export declare class _Worker {
     ...
     */
    capture(part: Position & Size, format: "monochrome", threshold?: number): Image;
    /**
     * Captures part of current workwindow (or screen if `handle` is `0`)
     * @param [format="rgba"] - color format of returned data, if not provided defaults to `"rgba"`
     */
    ...
    setWorkwindow(parentHandle: number, childClassName: Buffer | null, childTitle?: Buffer | null): void;
    setWorkwindow(parentTitle: Buffer | null, parentClassName: Buffer | null, childClassName: Buffer | null, childTitle?: Buffer | null): void;
...
export declare class _GlobalHotkey {
    protected _register(key: KeyboardRegularButton | number, action: () => Promise<void> | void): void;
...
export declare const _textToImg: (text: Buffer, path: Buffer, fontName: Buffer, fontSize: number, options: Required<TextToImgOptions>) => Image;
...
export declare function _getWindowChildren(parentTitle: Buffer | null, parentClassName?: Buffer | null): _WindowInfo[];
/** @returns string name of {@link virtualKey} */
export declare function vkToString(virtualKey: number): Exclude<KeyboardButton, number>;
export declare function getScreenSize(): Size;
export declare type Device = "mouse" | "keyboard";
export declare function isButtonPressed<D extends Device>(device: D, button: D extends "keyboard" ? KeyboardButton : MouseButton): boolean;
export declare type DisableInputOptions = {
    mouse?: (MouseButton | "wheel-forward" | "wheel-back" | "move")[];
    keyboard?: KeyboardButton[];
};
export declare type BlockedInput = {
    state: boolean;
} & ({
    device: "mouse";
    ...
    device: "keyboard";
    button: Exclude<KeyboardButton, number>;
});
/**
 ...
 */
export declare function disableInput(disable: true, options?: DisableInputOptions): void;
/**
 ...
 */
export declare function disableInput(disable: false, options?: DisableInputOptions): BlockedInput[];
export declare class _Hook {
    protected _register(device: Device, button: MouseButton | "wheel" | KeyboardButton, state: boolean, callback: () => void): void;
    protected _getButtonState(): boolean;
    delete(): void;
    static deleteAll(): void;
}
